...
**Upload icons to `public/assets/`** so they are served at `/assets/<filename>`. The full specification is in `public/assets/ICON_SPECIFICATION.md`.
...
Icons will be imported and used in components:

```tsx
import workspaceLogo from '@/assets/workspace-logo.svg'
import iconSearch from '@/assets/icon-search.svg'
```

Or referenced via URL if served from `public/`:
